...
Pre-trained models are included — you can run the demo without retraining.

```bash
cd blockait-main                        # or the path where you cloned/unzipped the repo
python -m venv .venv && source .venv/bin/activate   # Windows: .venv\Scripts\activate
pip install -r requirements.txt
streamlit run app.py
```

Open **[http://localhost:8501](http://localhost:8501)** in your browser (Streamlit’s default port).
The UI has three tabs: **Single complaint**, **Batch triage queue**, and **Model metrics**.
...
|---|---|
| **Category** (20 classes) | Trained multi-class classifier |
...
> fully offline. Trained model artifacts are committed under `models/`, so you can skip
> straight to the demo unless you want to regenerate data or retrain.

### Install & launch the Streamlit demo

```bash
cd blockait-main                        # project root (contains app.py)
...
Then open **[http://localhost:8501](http://localhost:8501)**. Streamlit prints the local URL
in the terminal; use a different port with `streamlit run app.py --server.port 8502` if 8501
is already taken.

**What you’ll see:** a Streamlit app with three tabs —
1. **Single** — type one complaint and get a colour-coded triage card.
2. **Batch** — run a sample or uploaded CSV through the triage queue.
3. **Metrics** — held-out test performance from `models/metrics.json`.

### Use without the UI (Python API)

The inference entry point is `classify_complaint()` in `pipeline.py`:

```python
from pipeline import classify_complaint

result = classify_complaint(
    "Возле школы №25 не работает уличный фонарь, вечером темно и опасно для детей"
)
print(result["category_label"], result["urgency"], result["priority_score"])
# -> Уличное освещение (Streetlight)  critical  84
```

The returned dict includes category, urgency, department, estimated cost/time, priority score,
confidence scores, `needs_human_review`, and a `priority_breakdown` for explainability.

### Optional semantic-embedding backend

Heavier (installs PyTorch; ~470 MB model download on first run with sentence-transformers):
...
| Issue | Fix |
|---|---|
| `Модели не найдены` / models not found in the UI | Run `python train.py` (after `pip install`); needs `models/category_clf.joblib` and siblings. |
| `ModuleNotFoundError` on import | Activate the venv and run `pip install -r requirements.txt` from the project root. |
| Port 8501 already in use | `streamlit run app.py --server.port 8502` and open `http://localhost:8502`. |
| First complaint is slow | Normal on first run — embeddings/models are warmed once (`app.py` caches them). |
| SBERT retrain fails or is slow | Use the default TF-IDF path (no `requirements-sbert.txt`) unless you need semantic embeddings. |
...
- **Project:** Blockait — SmartScape AI-for-Smart-Cities Hackathon (48-hour challenge).
...
**License:** no `LICENSE` file is included; treat as hackathon submission unless otherwise agreed with the authors.
