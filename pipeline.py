...
LANG_DISPLAY = {"ru": "Русский", "kk": "Қазақша", "en": "English"}

_MODELS = None           


def split_category_label(label: str):
    """Split 'RU label (English label)' into separate strings."""
    if "(" in label and label.endswith(")"):
        ru = label[: label.rindex("(")].strip()
        en = label[label.rindex("(") + 1 : -1].strip()
        return ru, en
    return label, label
...
def format_sla(hours: int) -> str:
    if hours < 24:
        return f"{hours} часа" if hours in (2, 3, 4) else f"{hours} часов"
    days = hours // 24
    if days == 1:
        return "1 день"
    if 2 <= days <= 4:
...
def classify_complaint(text: str) -> dict:
    ...
    cat_label_full = CATEGORY_LABELS.get(category, category)
    cat_label_ru, cat_label_en = split_category_label(cat_label_full)
    top_indices = np.argsort(cat_proba)[::-1]
    alternatives = []
    for idx in top_indices[1:3]:
        alt_cat = cat_enc.classes_[idx]
        alt_full = CATEGORY_LABELS.get(alt_cat, alt_cat)
        alt_ru, alt_en = split_category_label(alt_full)
        alternatives.append({
            "category": alt_cat,
            "label_ru": alt_ru,
            "label_en": alt_en,
            "confidence": round(float(cat_proba[idx]), 3),
        })
    ...
    return {
        ...
        "language": lang,
        "category": category,
        "category_label": cat_label_full,
        "category_label_ru": cat_label_ru,
        "category_label_en": cat_label_en,
        "category_alternatives": alternatives,
        "category_confidence": round(cat_conf, 3),
        "sla_hours": sla_hours,
        "sla_label": format_sla(sla_hours),
        "language_display": LANG_DISPLAY.get(lang, lang),
        "urgency": urgency,
